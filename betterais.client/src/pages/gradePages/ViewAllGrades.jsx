...
export default function ViewAllGrades() {
    ...
    useEffect(() => {
        ...
        const fetchGrades = async () => {
            try {
                ...
                console.error("Nepavyko gauti pažymių:", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) {
        return <div>Kraunami pažymiai...</div>;
    }

    return (
        <div className="container mx-auto p-6">
            <h2 className="text-2xl font-bold mb-4">Visi pažymiai</h2>
            {grades.length === 0 ? (
                <p>Pažymių nerasta.</p>
            ) : (
                <table className="table-auto w-full border-collapse border border-gray-300">
                    <thead>
                        <tr>
                            ...
                            <th className="border border-gray-300 px-4 py-2">Pažymys</th>
                            <th className="border border-gray-300 px-4 py-2">Data</th>
                            <th className="border border-gray-300 px-4 py-2">Suvestinės ID</th>
